...
import com.laioffer.demo.dao.FavoriteDao;
import com.laioffer.demo.entity.db.Item;
import com.laioffer.demo.entity.db.ItemType;
import com.laioffer.demo.entity.response.Game;
...
public class RecommendationService {
    ...
    private List<Item> recommendByFavoriteHistory(
        ...
        for (Map.Entry<String, Long> favoriteGame : sortedFavoriteGameIdListByCount) {
            ...
            for (Item item : items) {
                // if recommended item is equal to 20; return it
                if (recommendedItems.size() == DEFAULT_TOTAL_RECOMMENDATION_LIMIT) {
                    return recommendedItems;
                }
                // if the item is not included int the current favoritedItemIds, recommend it;
    ...
    public Map<String, List<Item>> recommendItemsByUser(String userId) throws RecommendationException {
        ...
        for (Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet()) {
            if (entry.getValue().size() == 0) {
                ...
                List<Item> items = recommendByTopGames(ItemType.valueOf(entry.getKey()), topGames);
                recommendedItemMap.put(entry.getKey(), items);
            } else {
                List<Item> items = recommendByFavoriteHistory(favoriteItemIds, entry.getValue(), ItemType.valueOf(entry.getKey()));
                recommendedItemMap.put(entry.getKey(), items);
            }
        }
